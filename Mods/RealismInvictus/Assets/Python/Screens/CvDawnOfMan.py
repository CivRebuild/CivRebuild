...
# HOF MOD
from Python.Components.BUFFY import Buffy
from Python.Components.BUG import BugCore
from Python.Components.BUFFY import GameSetUpCheck
...
# HOF MOD
        if (BUFFYOpt.isWarningsDawnOfMan()
            ...
            screen.addPanel(
                szMainPanel, "", "", True, True, self.X_MAIN_PANEL, self.Y_MAIN_PANEL,
                self.W_MAIN_PANEL, self.H_MAIN_PANEL + self.HOF_WARNING_PANEL_HEIGHT - 15,
                PanelStyles.PANEL_STYLE_MAIN
            )
            szHOFWarningPanel = "HOF Warning Panel"
            screen.addPanel(
                szHOFWarningPanel, "", "", True, True, self.HOF_WARNING_PANEL_X,
                self.HOF_WARNING_PANEL_Y, self.HOF_WARNING_PANEL_WIDTH,
                self.HOF_WARNING_PANEL_HEIGHT, PanelStyles.PANEL_STYLE_MAIN
            )
            szHOFWarningBox = "HOF Warning Box"
            screen.addListBoxGFC(
                szHOFWarningBox, "", self.HOF_WARNING_PANEL_X + 10,
                self.HOF_WARNING_PANEL_Y + 10, self.HOF_WARNING_PANEL_WIDTH - 20,
                self.HOF_WARNING_PANEL_HEIGHT, TableStyles.TABLE_STYLE_EMPTY
            )
            screen.enableSelect(szHOFWarningBox, False)
            screen.appendListBoxString(
                szHOFWarningBox, GameSetUpCheck.getWarningMessage(),
                WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY
            )
        else:
            screen.addPanel(
                szMainPanel, "", "", True, True, self.X_MAIN_PANEL, self.Y_MAIN_PANEL,
                self.W_MAIN_PANEL, self.H_MAIN_PANEL, PanelStyles.PANEL_STYLE_MAIN
            )
# end HOF MOD
