...
def getModInfo( mapVersion=None, defLatitude=None, sMapInfo=None, bNoSigns=False ):
    ...
    sVersion = str(CyTranslator().getText("TXT_KEY_BTS_CIVS", ()))					# check for BtS
#	print "[MST] " + sVersion
    bBTS = (sVersion.find("Beyond the Sword") > -1)

    sVersion = str(CyTranslator().getText("TXT_KEY_MAIN_MENU_WARLORS", ()))		# check for Warlords
#	print "[MST] " + sVersion
    bWarlords = (not bBTS) and (sVersion.find("Warlords") > -1)

    sVersion = str(CyTranslator().getText("TXT_KEY_MAIN_MENU_ABOUT_CIV4", ()))	# check for Vanilla
#	print "[MST] " + sVersion
    ...
    sTxt = str(CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_CONCEPT_NEW", ()))		# 'Mars Now!'
    ...
    sVersion = str(CyTranslator().getText("TXT_KEY_VERSION", ())) # Many mods provide a TXT with the version.
    ...
    # FFH - Fall Flat
    sVersion = str(CyTranslator().getText("TXT_KEY_VERSION", ()))		# Fall Flat does it right !
    ...
    sVersion = str(CyTranslator().getText("TXT_KEY_CONCEPT_LOR_REVOLUTIONS_PEDIA", ()))
    bLoR = (sVersion.find("Legends of Revolution") > -1)

    # Rise of Mankind
    sVersion = str(CyTranslator().getText("TXT_KEY_CONCEPT_ROM_RELIGION_PEDIA", ()))
    bRoM = (sVersion.find("Rise of Mankind") > -1)

    # Thomas' War
    sVersion = str(CyTranslator().getText("TXT_KEY_NEW_CIV", ()))
    bTW = (sVersion.find("Thomas' War") > -1)

    # The History of Three Kingdoms
    sVersion = str(CyTranslator().getText("TXT_KEY_BUG_CREATED_BY_HEADER", ()))
    bHoTK = (sVersion.find("History of Three Kingdoms") > -1)

    # History in the Making
    sVersion = str(CyTranslator().getText("TXT_KEY_DCM_TEXT", ()))
    bHitM = (sVersion.find("History in the Making") > -1)

    # Overlord
    sVersion = str(CyTranslator().getText("TXT_KEY_GAMESPEED_FXOVERLORD", ()))
    ...
    # Community Civ V
    sVersion = str(CyTranslator().getText("TXT_KEY_CCV_MOD_RUNNING_INFO", ()))
    ...
    if not bInitialized:
        ...
        sprint += "[MST] " + "==============================%s \n" % ( '='*iName )
        print (sprint)
        print ("[MST] " + ("#"*68) + "MapScriptTools:getModInfo() ###")
...
def checkAIAuto():
    txt = str(CyTranslator().getText("TXT_KEY_AIAUTOPLAY_TURNS", ()))
    return txt.find("AI to control") > -1

# test if BBAI was incorporated into mod
def checkBBAI():
    txt = str(CyTranslator().getText("TXT_KEY_MISC_SHIFT_ALT_PREPARE_WAR", ()))
    return txt.find("War Plan") > -1

# test if BUG was incorporated into mod
def checkBUG():
    txt_0 = str(CyTranslator().getText("TXT_KEY_BUG_OPTIONS_KEY_REMINDER", ()))
    txt_1 = str(CyTranslator().getText("TXT_KEY_BUG_OPTTAB_CREDITS", ()))
    return ( txt_0.find("BUG Mod") > -1 ) or ( txt_1.find("Credits") > -1 )

# test if RevolutionDCM was incorporated into mod
def checkRevDCM():
    txt = str(CyTranslator().getText("TXT_KEY_REVOLUTION_WATCH_ADVISOR_TITLE", ()))
                ...
                if self.isTrench( plot ):
                    ...
                    if terCnt == 0:
                        if plot.getFeatureType() == efTrench:
                            ...
                            cntKillTrenches += 1
        print (
            "[MST] Connected %i Trenches - Eliminated isolated Shelves: %i, Trenches: %i" % (
                cntAddTrenches, cntKillShelves, cntKillTrenches
            )
        )
                ...
                for eDir in edList:
                    ...
                    if (map.getArea(plot.getArea()).getNumTiles() == 1):
                        if choose(chConnect, True, False):
                            plot.setPlotType(
                                choose(90,PlotTypes.PLOT_LAND,PlotTypes.PLOT_HILLS),
                                False, False
                            )
                            lCnt += 1
                            break
...
#		if bPfall: return
            ...
            for y in range( y0, y1 ):
                ...
                if plot.getFeatureType() == efVolcano:
                    ...
                    if plot.isCoastalLand():
                        if choose( chHills, True, False ):
                            plot.setPlotType(
                                choose(chHills,PlotTypes.PLOT_HILLS,PlotTypes.PLOT_PEAK),
                                True, True
                            )
                            ...
                            if feat != None:
                                ...
                                cntVol += 1
        if cntVol > 0: print (
            "[MST] %i coastal volcanos eliminated and transformed to hills/peaks" % ( cntVol )
        )
    ...
    def lumpifyTerrain( self, targetTerrain, sourceTer1, sourceTer2=None ):
        print (
            "[MST] ===== MapPrettifier:lumpifyTerrain(%r, %r, %r)" % (
                targetTerrain, sourceTer1, sourceTer2
            )
        )
        cnt = 0
        chChange = 40
    ...
    def lumpifyFeature( self, targetFeature, sourceFeat1, sourceFeat2=None, passes=1 ):
        print (
            "[MST] ===== MapPrettifier:lumpifyFeature(%r, %r, %r)" % (
                targetFeature, sourceFeat1, sourceFeat2
            )
        )
        cnt = 0
        chChange = 40
    ...
    def percentifyTerrain( self, targetTerTuple, *sourceTerTuples ):
        ...
        for loop in passes:
            ...
            if loop == 0:
                print (
                    "[MST] Target percentage: %i, Terrain percentage before: %4.2f(%i)" % (
                        chPercent,cntTer*100.0/baseTer,cntTer
                    )
                )
            ...
            if	wantTer < cntTer:
                ...
                for inx in terList:
                    ...
                    if not (newTer == None):
                        ...
                        if	wantTer == cntTer:
                            print (
                                "[MST] Target percentage reached. \n "
                                "Target percentage: %i, Terrain percentage after %i "
                                "passes: %4.2f(%i)" % (
                                    chPercent,loop+1,cntTer*100.0/baseTer,cntTer
                                )
                            )
                            return
            else:
                ...
                for inx in srcList:
                    ...
                    if	wantTer == cntTer:
                        ...
                        return
        print (
            "[MST] Target percentage: %i, Terrain percentage after %i passes: %4.2f(%i)" % (
                chPercent,loop,cntTer*100.0/baseTer,cntTer
            )
        )
    ...
    def percentifyPlots( self, targetPlotType, fTargetPlotPercent, data=None, terGenerator=None ):
        print (
            "[MST] ===== MapPrettifier:percentifyPlots(%r, %5.3f, %r, %r)" % (
                targetPlotType, fTargetPlotPercent,
                iif(data,"data",data),
                iif(terGenerator,"terGenerator",terGenerator)
            )
        )
        sourceDict = {
        ...
        for loop in passes:
            ...
            if loop == 0:
                print (
                    "[MST] Target percentage: %4.2f, plotType percentage before: %4.2f(%i)" % (
                        fTargetPlotPercent,cntPlot*100.0/map.numPlots(),cntPlot
                    )
                )
            ...
            if	wantPlots < cntPlot:
                ...
                for inx in plotList:
                    ...
                    if	abs(wantPlots - cntPlot) <= 1:
                        print (
                            "[MST] Target percentage reached. \n "
                            "Target percentage: %4.2f, PlotType percentage after %i "
                            "passes: %4.2f(%i)" % (
                                fTargetPlotPercent,loop+1,cntPlot*100.0/map.numPlots(),cntPlot
                            )
                        )
                        return data
            else:
                ...
                for inx in srcList:
                    ...
                    if	abs(wantPlots - cntPlot) <= 1:
                        ...
                        return data
        print (
            "[MST] Target percentage: %4.2f, PlotType percentage after %i passes: %4.2f(%i)" % (
                fTargetPlotPercent,loop,cntPlot*100.0/map.numPlots(),cntPlot
            )
        )
        return data
...
class MarshMaker:
    ...
    def initialize( self, iGrassChance=5, iTundraChance=10, tMarshHotRange=(0,18), tMarshColdRange=(45,63) ):
        print (
            "[MST] ===== MarshMaker:initialize( %r, %r, %r, %r )" % (
                iGrassChance, iTundraChance, tMarshHotRange, tMarshColdRange
            )
        )
    ...
    def convertTerrain( self, tAreaRange=None, areaID=None ):
        ...
        print (
            "[MST] %i plots converted to Marsh: %i from Grass, %i from Tundra" % (
                self.cntGrass+self.cntTundra,self.cntGrass,self.cntTundra
            )
        )
...
class MapRegions:
    ...
    def initialize( self, regDist=15, noSigns=False ):
        print (
            "[MST] ===== MapRegions:initialize( regDist=%r, noSigns=%r )" % (
                regDist, noSigns
            )
        )
        # class
    ...
    def buildLostIsle( self, chance=33, minDist=8, bAliens=False ):
        ...
        print (
            "[MST] ===== MapRegions:buildLostIsle(%r,%r,%r)" % (chance, minDist, bAliens)
        )
...
#		print "[MST] ======== MapRegions:theBigBog()"
    ...
    def buildBigDents( self, iDents=2, chDent=50 ):
        ...
        if not (bMars or bPfall):
            dentNames.append({
                    'Volcano': "TXT_KEY_MAPSCRIPTTOOLS_DENT_6_VOLCANO",
            ...
            dentNames.append({
                'Volcano': "TXT_KEY_MAPSCRIPTTOOLS_DENT_PLANETFALL_VOLCANO",
            ...
            dentNames.append({
                'Volcano': "TXT_KEY_MAPSCRIPTTOOLS_DENT_MARSNOW_VOLCANO",
        ...
        while (map.getWorldSize() > minMap) and (maxDent > nDent):
            ...
            if choose( chDent/(minMap-1), True, False ):
                ...
                # get valid plots
                validPlots = [
                    p for num,id,pList in continents
                    for p in pList
                    if self.regionCheck( p ) and self.wrapCheck( p )
                ]
                # build next dent
...
#		printList( template, "rotated %i" % (rot), rows=1, prefix="[MST] " )
        print (
            "[MST] Template rotated %i degrees, sideways: %r, rotate: %r" % (
                rot*90, sideways, rotate
            )
        )
...
#							sprint += "[MST] @ %3i,%2i: Hills -> Peak \n" % ( plot.getX(), plot.getY() )
        ...
        if not bSandsOfMars:
            ...
            # calc river origins
            lx = list(range(x0, x0+3))
            ly = list(range(y0, y0+3))
            if bSide:
                lx.append( x0+3 )
            else:
                ly.append( y0+3 )
        ...
        for x in randomList.xshuffle( list(range(minX-3, maxX+4))):
            for y in randomList.xshuffle( list(range(minY-3, maxY+4))):
                plot = map.plot(x, y)
                if plot.isNone(): continue
                if plot.isWater():
                    if isCoastalWater(x, y):
            ...
            for i in range(4):
                ...
                if choose( chance, True, False ):
                    ...
                    impList = [
                        (bonEra, imp)
                        for imp
                        in range(gc.getNumImprovementInfos())
                        if canHaveImprovement(pl, imp, True)
                        and gc.getImprovementInfo(imp).isImprovementBonusMakesValid(bon)
                    ]
                    if len(impList) == 0:
...
class FeaturePlacer:
    ...
    def placeReefs( self, chReef=3, chExpand=20, maxLatitude=None ):
        ...
        print (
            "[MST] ===== FeaturePlacer:placeReefs( %i, %i, %r )" % (
                chReef, chExpand, maxLatitude
            )
        )
    ...
    def expandReef( self, x, y, chanceExpand = 40, badDir = -1, lastDir = -1, reefList = None ):
        ...
        print (
            "[MST] -- FeaturePlacer:expandReef( %i:%i, %i, %i, %i, %r )" % (
...
class BonusBalancer:
    ...
    resourcesToBalance   = (
        ...
        'BONUS_IRON', 'BONUS_OIL', 'BONUS_URANIUM', 'BONUS_SALTPETER',
    )
    resourcesToEliminate = ('', )
    mineralsToMove       = (
        'BONUS_COPPER', 'BONUS_IRON', 'BONUS_SILVER', 'BONUS_GOLD',
        'BONUS_URANIUM', 'BONUS_SALTPETER',
    )

    def initialize(self, bBalanceOnOff=True, bMissingOnOff=True, bMineralsOnOff=True, bWideRange=False ):
        print (
            "[MST] ===== BonusBalancer:initialize( %r, %r, %r, %r )" % (
                bBalanceOnOff, bMissingOnOff, bMineralsOnOff, bWideRange
            )
        )
        ...
        for i in range( map.numPlots() ):
            ...
            if iBonus>=0:
                ...
                if type_string in self.resourcesToEliminate:
                    ...
                    iCnt += 1
        print (
            "[MST] Eliminated %2i of the boni to be eliminated:\n %r" % (
                iCnt, self.resourcesToEliminate
            )
        )
            ...
            if iBonus>=0:
                ...
                if type_string in self.resourcesToBalance:
                    ...
                    if choose(iChoice,True,False):
                        ...
                        iCnt += 1
        print (
            "[MST] Eliminated %2i wild boni of those to be balanced (%i%s chance)\n" % (
                    ...
                    if pDist == dis:
                        ...
                        if bDownFlow:
                            ...
                            if ( len(rivDirs) == 1 ) or ( len(rivDirs) == 2 ) or plot.isRiver():
                                rPlots.append( plot )
        print (
            "[MST] %i possible %s found" % (
                len(rPlots),iif(bDownFlow, "hilly starts", "river-ends")
            )
        )
...
class TeamStart:
    ...
    def placeTeamsTogether( self, bTogether=False, bSeparated=False ):
        ...
        if bTogether and bSeparated:
            print (
                "[MST] All players to be randomly distributed over available starting-plots."
            )
            plotList = []
...
#		print "[MST] ===== MapPrint:buildDiffMap()"
        ...
        if newDict[ 'TITLE' ] != oldDict[ 'TITLE' ]:
            print (
                "[MST] Error! Different map type: %r - %r (new,old)" % (
                    newDict['TITLE'], oldDict['TITLE']
                )
            )
            return False
        # check region
        if newDict[ 'REGION' ] != oldDict[ 'REGION' ]:
            print (
                "[MST] Error! Different region: %r - %r (new,old)" % (
                    newDict['REGION'], oldDict['REGION']
                )
            )
            return False
        # check length
        if len(newDict) != len(oldDict):
            print (
                "[MST] Error! Different dict-length: %i, %i (new,old)" % (
                    len(newDict), len(oldDict)
                )
            )
            return False
        ...
        for lin in newDict.keys():
            ...
            if len(newDict[lin]) != len(oldDict[lin]):
                print (
                    "[MST] Error! Different line-length in dicts. Line: %r (new %i, old %i)" % (
                        lin,len(newDict[lin]),len(oldDict[lin])
                    )
                )
                printDict( newDict[lin], "New Map", prefix="[MST] " )
                printDict( oldDict[lin], "Old Map", prefix="[MST] " )
...
class MapOptionsStorage:
    ...
    def readConfig( self ):
        ...
        print (
            "[%s] -- MapOptionsStorage.readConfig() - already called? %r" % (
                self.sMapScript, self.hasReadConfig
            )
        )
        ...
        if len(lMapOptions) != len(self.lDefaults):
            print (
                "[%s] The stored map options size does not follow the "
                "same format than the default list. Using default list." % self.sMapScript
            )
            return self.lDefaults
    ...
    def writeConfig( self, lMapOptions ):
        ...
        print (
            "[%s] -- MapOptionsStorage.writeConfig(): Written file %r" % (
                self.sMapScript, fileName
            )
        )
        settings.close()
        self.hasReadConfig = False
